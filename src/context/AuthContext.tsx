...
import { doc, getDoc, setDoc, onSnapshot, collection, deleteDoc, writeBatch, query, where, getDocs } from 'firebase/firestore';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addAdmin = async (email: string, role: AdminRole, categories: string[] = []) => {
    // Require an existing Shoppa account for this email before granting admin
    // rights. Without this check, a Super Admin could pre-provision an email
    // that hasn't signed up yet — and since Firebase allows signing up with
    // any email without verifying ownership of it, someone else could sign up
    // with that exact email first and inherit the admin role instantly.
    const trimmedEmail = email.trim();
    const profilesQuery = query(collection(db, 'profiles'), where('email', '==', trimmedEmail));
    const matchingProfiles = await getDocs(profilesQuery);
    if (matchingProfiles.empty) {
      throw new Error('No Shoppa account exists for this email yet. Ask them to sign up first, then add them as an admin.');
    }

    const adminsCollection = collection(db, 'admins');
    await setDoc(doc(adminsCollection, trimmedEmail), { email: trimmedEmail, role, managedCategories: categories });
  };
  ...
  const fundAccount = async (amount: number) => {
    if (user && amount > 0) {
      ...
      // NOTE: this write is expected to fail under current Firestore rules —
      // wallet balance can no longer be set directly by the client (see
      // firestore.rules). Wallet funding needs a real payment gateway +
      // server-side balance mutation before this can work again. Callers
      // must catch and handle the rejection; see FundWalletClient.tsx.
