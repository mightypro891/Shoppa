...
export default function SignUpForm() {
  ...
  const handleSuccessfulLogin = (isNewUser: boolean) => {
    ...
    // New users always go to the welcome page to complete their profile.
    if (isNewUser) {
  ...
  return (
    ...
    <Card className="w-full max-w-md border-white/10 bg-white/[0.04] backdrop-blur-2xl shadow-[0_0_60px_-15px] shadow-primary/30">
      <CardHeader className="text-center">
        ...
        <CardDescription className="text-white/50">Enter your details to get started.</CardDescription>
      </CardHeader>
      <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField
                    ...
                    render={({ field }) => (
                        ...
                        <FormControl>
                            <Input placeholder="John Doe" className="bg-white/5 border-white/10 text-white placeholder:text-white/30 focus-visible:ring-primary/40 focus-visible:border-primary/50" {...field} />
                        </FormControl>
                        <FormMessage />
                        </FormItem>
                    )}
       ...
       <CardFooter className="flex justify-center text-sm text-white/50">
            ...
            <Link href="/auth/signin" className="text-primary hover:text-primary/80 hover:underline">
             Sign In
            </Link>
      </CardFooter>
    </Card>
