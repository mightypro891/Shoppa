...
import { Package, Star, Megaphone, ShoppingCart, Users, Trash2, PlusCircle, ChevronDown, Wifi, PartyPopper, ThumbsUp, ThumbsDown, Check, X, MapPin } from 'lucide-react';
import Link from 'next/link';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../ui/tabs';
import dynamic from 'next/dynamic';

const ProductPieChart = dynamic(() => import('./charts/ProductPieChart'), {
  ssr: false,
  loading: () => <div className="h-[300px] w-full animate-pulse rounded-lg bg-muted" />,
});
const ProductBarChart = dynamic(() => import('./charts/ProductBarChart'), {
  ssr: false,
  loading: () => <div className="h-[300px] w-full animate-pulse rounded-lg bg-muted" />,
});
...
export default function AdminDashboard() {
  ...
  const handleAddAdmin = async () => {
    if (newAdminEmail && /\S+@\S+\.\S+/.test(newAdminEmail)) {
        try {
          await addAdmin(newAdminEmail, newAdminRole, selectedCategories);
