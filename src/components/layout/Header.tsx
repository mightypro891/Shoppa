                      ...
                      <div className="grid grid-cols-3 gap-x-6 gap-y-4 p-6 w-[750px] lg:w-[900px]">
                        {mainCategories.map((category) => (
                          <div key={category.slug} className="flex flex-col space-y-2">
                            <NavigationMenuLink asChild>
                                <Link href={`/products/category?category=${category.slug}`} className="font-semibold text-base mb-1 pb-1 border-b border-primary/50 hover:text-primary transition-colors">
                                    {category.name}
                                </Link>
                            </NavigationMenuLink>
                            <ul className="flex flex-col gap-1.5">
                                {category.subcategories?.map(subcategory => (
                                    <li key={subcategory.slug}>
                                        <NavigationMenuLink asChild>
                                            <Link href={`/products/category?category=${subcategory.slug}`} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                                                {subcategory.name}
                                            </Link>
                                        </NavigationMenuLink>
                                    </li>
                    ...
                    <DropdownMenuSubContent>
                      {mainCategories.map((category) => (
                         <DropdownMenuItem key={category.slug} asChild>
                             <Link href={`/products/category?category=${category.slug}`}>{category.name}</Link>
                         </DropdownMenuItem>
                      ))}
                    </DropdownMenuSubContent>
                  </DropdownMenuPortal>
          ...
          <Button asChild variant="ghost" size="icon" className="relative">
            <Link href="/cart">
              ...
              {itemCount > 0 && (
                <span
                  key={itemCount}
                  className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground animate-pop"
                >
                  {itemCount}
                </span>
              )}
